...
pub const EvaluationSession = struct {
    ...
    pub fn evaluate(self: *Self, target: EvaluationTarget) !EvaluationSummary {
        ...
        for (self.test_cases.items[0..sample_size]) |test_case| {
            ...
            // Record latency
            var latency_result = try EvaluationResult.init(self.allocator, .latency, "response_time", latency_ms);
        ...
        var accuracy_result = try EvaluationResult.init(self.allocator, .accuracy, "overall_accuracy", accuracy);
        try self.results.append(accuracy_result);
        
        var avg_latency_result = try EvaluationResult.init(self.allocator, .latency, "average_latency", avg_latency);
        ...
        std.log.info("Evaluation completed: {d} test cases, {d:.2}% accuracy, {d:.2}ms avg latency", 
                    .{ sample_size, accuracy * 100.0, avg_latency });
    ...
    pub fn evaluateRAG(self: *Self, rag_system: anytype, queries: [][]const u8) !RAGEvaluationResult {
        ...
        const avg_relevance = if (relevance_scores.items.len > 0) 
            self.calculateAverage(relevance_scores.items) else 0.0;
        const avg_latency = if (response_times.items.len > 0) 
            self.calculateAverage(response_times.items) else 0.0;
    ...
    fn saveResults(self: *Self) !void {
        ...
        for (self.results.items, 0..) |result, i| {
            ...
            try writer.print("{{\"name\":\"{s}\",\"value\":{d},\"timestamp\":{d}}}", 
